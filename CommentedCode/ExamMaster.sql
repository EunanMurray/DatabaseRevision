    ...
    BEGIN TRY
        ...
        BEGIN
            BEGIN TRY
                ...
                BEGIN
                    -- Find the first invalid member for error message
                    ...
                    SET @ErrorMessage = 'Sorry, Member ' + CAST(@InvalidMemberID AS NVARCHAR) + ' does not have an active membership plan';
                ...
                BEGIN
                    -- Find the first full class for error message
                    SELECT TOP 1 @FullClassID = cs.ScheduleID
                    FROM @Bookings b
                    JOIN ClassSchedule cs ON b.ScheduleID = cs.ScheduleID
                    WHERE cs.CurrentBookings >= cs.MaxCapacity;
                    
                    SET @ErrorMessage = 'Sorry, class ' + CAST(@FullClassID AS NVARCHAR) + ' is already at full capacity';
                    RAISERROR(@ErrorMessage, @ErrorSeverity, @ErrorState);
                    RETURN;
                END
                ...
                BEGIN
                    -- Find the first duplicate booking for error message
                    SELECT TOP 1 
                    ...
                    SET @ErrorMessage = 'Member ' + CAST(@DupMemberID AS NVARCHAR) + ' has already booked class ' + CAST(@DupScheduleID AS NVARCHAR);
                    RAISERROR(@ErrorMessage, @ErrorSeverity, @ErrorState);
                    RETURN;
                END
